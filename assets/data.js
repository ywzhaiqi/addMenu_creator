...
var Data = {
    ...
    aCloseOtherClick_tab: {
        _message: ' onclick 中包含 CloseRepeatedTabs 函数，可能无效',
    ...
    aOpenWith_chrome_app: {
        label: "在 Chrome 中打开",
        tooltiptext: "在 Chrome 中打开当前链接 / 页面",
        accesskey: "C",
        text: '%RLINK_OR_URL%',
        exec: "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe",
    },
    aOpenWith_chrome_disalbe_extension_app: {
        label: "用 Chrome 打开当前页面(禁用扩展)",
        tooltiptext: "包括当前页面和当前链接",
        text: "%RLINK_OR_URL% -disable-extensions",
        exec: 'Services.dirsvc.get("LocalAppData", Ci.nsILocalFile).path + "\\Google\\Chrome\\Application\\chrome.exe"',
        accesskey: "E",
    },
    aOpenWith_7chrome_app: {
        label: "在 7Chrome 中打开",
        tooltiptext: "在 7Chrome 中打开当前链接 / 页面",
        text: "%RLINK_OR_URL%",
        exec: 'Services.dirsvc.get("LocalAppData", Ci.nsILocalFile).path + "\\7Star\\7Star\\Application\\7chrome.exe"'
    },
    aOpenWith_TW6_app: {
        label: "在 TW6 中打开",
        tooltiptext: "在 TW6 中打开当前链接 / 页面",
        accesskey: "T",
        text: "%RLINK_OR_URL%",
    ...
    aOpenWith_Maxthon_app: {
        label: "在 Maxthon 中打开",
        tooltiptext: "在 Maxthon 中打开当前链接 / 页面",
        text: "%RLINK_OR_URL%",
        exec : "D:\\Program Files\\Maxthon\\Bin\\Maxthon.exe",
    },
    aOpenWith_Opera_app: {
        label: "在 Opera 中打开",
        tooltiptext: "在 Opera 中打开当前链接 / 页面",
        text : "%RLINK_OR_URL%",
    ...
    aVideoDownload_byIDM: {
        label: '用 IDM 直接下载视频',
        id: 'addMenu-idm-video',
        image: 'moz-icon://file:///d:\\Program Files\\Internet Download Manager\\IDMan.exe?size=16',
        oncommand: function() {
                ...
                xhr.onload = function(){
                    if(this.readyState == 4 && this.status == 200){
                        var doc = this.response,
                            title = doc.querySelector('input[name="name"]').getAttribute("value"),
                            urls = doc.querySelector('input[name="inf"]').getAttribute("value").split('|');
            ...
            function runIDM(title, urls, notActive, fileExt, start) {
                // 调用 IDM
                ...
                var onlyOne = (urls.length == 1);
                if (!fileExt)
                    fileExt = getVideoExt(urls[0]);
                var run = function (i) {
                    var url = urls.shift();
                    if (!url) {  // 再次运行，激活到前台
                        if (!notActive)
                    ...
                    var fileName = onlyOne ? title : (title + '-' + (i < 10 ? '0' + i : i));
                    fileName = safeTitle(fileName) + fileExt;
                    var args = ["/a", "/d", url, "/f", fileName];
