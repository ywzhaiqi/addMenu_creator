...
var menuCategories = menu.styles;
...
var Menu = function() {
    var self = this;
    self.category = ko.observable();
    self.lines = ko.observableArray([new MenuLine()]);
    self.grandTotal = ko.pureComputed(function(){
        return self.lines().length;
    });

    // Operations
    self.addLine = function() { self.lines.push(new MenuLine()) };
    self.removeLine = function(line) { self.lines.remove(line) };
    self.save = function() {
        var dataToSave = $.map(self.lines(), function(line) {
            return line.product() ? {
                menuLabel: line.command().label,
            } : undefined
        });
