...
var menu = {
    styles: [  // 菜单类型
        ...
        { name: '图标菜单', value: 'MenuGroup', title: '类似 firefox 32 右键的前进后退图标菜单，需要 ff31+' },
    ],
    types: [  // 命令类型
        { name: '所有', value: '' },
        { name: '复制', value: 'copy' },
...
var MenuCtrl = Class.extend({
    ...
    tpl: {
        ...
        menuGroup: 'menu = {style}({ label: "自定义图标菜单" });\nmenu([{value}]);',
    },
    messages: [],
    selectedCommands: [],
    ...
    styleChanged: function(e) {
        ...
        // 因为 rivets.js 在 firefox 下会延迟得到值的 bug，所以用 jQuery 方式实现
        this.isMenu = this.style.toLowerCase().indexOf('menu') !== -1;
        this.isMenuGroup = this.style == 'MenuGroup';
    },
    ...
    addText: function(scripts) {
        var tplStr;
        if (this.isMenuGroup) {
            tplStr = this.tpl.menuGroup;
        } else if (this.isMenu) {
            tplStr = this.tpl.menu;
        } else {
